<script>
	...
	function mouseupHandler(ev) {
		...
		// start editing only if element was already focused on mousedown and no dragging was done
	...
	// this will init focused variable
	$: if (dragElement && editElement) {
		updateFocused()
	}
	async function updateFocused() {
		await tick()
	...
	$: {
		style = mainStyle ?? ''
		style += (dragFocused || editFocused) && focusStyle ? ';' + focusStyle : ''
	...
	function stopEditing() {
		if (editing) {
			...
			// Bring focus back to the drag element if editElement was focused:
